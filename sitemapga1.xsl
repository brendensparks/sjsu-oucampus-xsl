...
<xsl:variable name="sjsuwebsiteurl">
    <xsl:choose>
      ...
      <xsl:when test="not($ou:websiteurl)"> 
        <xsl:value-of select="'www.sjsu.edu/'" />
      </xsl:when>
...
<xsl:template name="file" xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
	...
	<xsl:choose> <!-- choose if in root or not -->
	<xsl:when test="$currentPath = ''">
			
		<xsl:choose>
			<xsl:when test="doc-available(concat($root,'/',.))">
			...
			<xsl:otherwise>
			    <!-- pcf file not available -->
				
				<xsl:comment> not available <xsl:value-of select="concat($root,'/',.)"/></xsl:comment>			
			</xsl:otherwise>
		
		</xsl:choose>
	
		
	</xsl:when> <!-- end if currentPath blank - in root -->
	<xsl:otherwise> <!-- not in root -->
		<xsl:choose>
		
			<xsl:when test="doc-available(concat($root,'/',$currentPath,'/',.))">
		  ...
		  <xsl:otherwise>
			  ...
			  <xsl:comment> not available <xsl:value-of select="concat($root,'/',$currentPath,'/',.)"/></xsl:comment>			
			</xsl:otherwise>
	...
	</xsl:choose><!-- end if in root -->
	</xsl:if> <!-- end if .pcf file -->
