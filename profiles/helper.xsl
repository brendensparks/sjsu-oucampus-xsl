...
exclude-result-prefixes="xs ou ouc">
	...
	<xsl:function name="ou:getProfile">
		<xsl:param name="profile" />
		<profile href="{concat($dirname, replace($ou:filename, '.xml', '.html'))}">
			<xsl:for-each select="$profile/ouc:div">
				<xsl:element name="{translate(./@label, ' ', '_')}"><xsl:apply-templates select="node()[not(self::ouc:multiedit)]" mode="copy" /></xsl:element>
			</xsl:for-each>
		</profile>
	</xsl:function>
	
	<xsl:function name="ou:getProfiles">
		<xsl:param name="dir" />
		<xsl:param name="skip" />
		
		<xsl:variable name="profiles">
			<xsl:for-each select="doc(concat($root, $ou:dirname))/list/file[contains(., '.pcf')]">
