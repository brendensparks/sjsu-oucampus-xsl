...
<xsl:variable name="httpsite_slash" as="xs:string" select="substring-after($ou:httproot, '.edu/')" />
<xsl:variable name="httpsite" as="xs:string" select="substring-before($httpsite_slash, '/')" />
...
<xsl:template name="bc-folders">
	...
	<xsl:choose>
		<xsl:when test="($href = '/')">
			<xsl:value-of select="concat('/',$httpsite,$path, '/')" />
		</xsl:when>
	...
	<xsl:choose>
		<xsl:when test="$path = '/' and $ou:sitepath = ''">
			<a href="http://www.sjsu.edu/">SJSU Home&#160;&gt;&#160;</a> <a href="{$ou:httproot}"><xsl:value-of select="$ou:subsite" /></a>
		</xsl:when>	
		<xsl:when test="$path = '/' and $ou:sitepath != ''">
			<a href="http://www.sjsu.edu/">SJSU Home&#160;&gt;&#160;</a> <a href="{$ou:sitepath}"><xsl:value-of select="$ou:subsite" /></a>
		</xsl:when>
		<xsl:otherwise>
			&gt;&#160;<a href="{$bchref}"><xsl:value-of select="doc(concat($root, $href, $file))/document/title/text()"/></a>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
