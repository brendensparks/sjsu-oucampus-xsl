			...
			<div id="top_subsite">
			        <div class="left">
			        	...
			        	<xsl:if test="$ou:encunitlink != ''">
							<a href="{$ou:encunitlink}">
								<span class="subsite_enclosingunit"><xsl:value-of select="$ou:enclosingunit"/></span>
							</a>
						</xsl:if>
			        </div>
	        <div class="right">		        
		...
		<div id="quickicons"> 
			...
			<div id="content" role="main">
				...
				<nav class="secnav">
		    		...
		    		<xsl:choose>
		    			<xsl:when test="(document/config/parameter[@name='pagetype'] = 'forms_php') or (document/config/parameter[@name='pagetype'] = 'siteindex')" >
		    			...
		    			<ul id="currnav">
							...
							<xsl:processing-instruction name="php"> $directoryName = "<xsl:value-of select="$dirname" />"; include($_SERVER["DOCUMENT_ROOT"] . $directoryName . "sidenav.inc"); ?</xsl:processing-instruction>
							<xsl:comment> /com.omniupdate.div </xsl:comment>
						</ul>
						<ul id="prevnav">
						<xsl:if test="($dirname != '/')" >
							<xsl:comment> com.omniupdate.div label="parentnav" path="<xsl:value-of select="$parent-path"/>/sidenav.inc" </xsl:comment> 
							<xsl:processing-instruction name="php"> $directoryName = "<xsl:value-of select="$parent-path" />"; include($_SERVER["DOCUMENT_ROOT"] . $directoryName . "/sidenav.inc"); ?</xsl:processing-instruction>
							<xsl:comment> /com.omniupdate.div </xsl:comment>
						</xsl:if>
						</ul>
						</xsl:otherwise>
