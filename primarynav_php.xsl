...
<xsl:template match="/">
<ul id="topnav">
		<xsl:call-template name="topnavphp">	
						...
						<xsl:when test="doc($child)/list/directory != ''">
							<li>
								<a>
						 		...
						 		<div class="sub">
						 			<ul>
						 				<xsl:comment> com.omniupdate.div label="<xsl:value-of select="."/>" path="<xsl:value-of select="concat('/',.,'/sidenav.inc')"/>" </xsl:comment>
						 				<xsl:processing-instruction name="php"> 
											$directory = "<xsl:value-of select="concat($ou:sitepath,'/',.,'/sidenav.inc')"/>";
						 					include($_SERVER['DOCUMENT_ROOT'] . $directory); 
