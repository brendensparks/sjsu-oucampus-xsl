...
<body>
	<xsl:choose>
		<xsl:when test="(document/config/parameter[@name='pagetype'] = 'news_php')">
...
<a href="{$ou:encunitlink}"><span class="subsite_enclosingunit"><xsl:value-of select="$ou:enclosingunit"/></span></a>
		...
		<xsl:when test="doc(concat($root, '/includes'))/list/file[. eq 'meganav.inc']">	
			...
			<xsl:processing-instruction name="php"> include($_SERVER["DOCUMENT_ROOT"] . "<xsl:value-of select="$ou:sitepath"/>/includes/meganav.inc"); ?</xsl:processing-instruction>
			<xsl:comment> /com.omniupdate.div </xsl:comment>
		</xsl:when>
		<xsl:otherwise>
 			<xsl:if test="$ou:action = 'prv' or $ou:action = 'edt'">
		...
		<div id="quickicons"> 
			...
			<div id="content" role="main">
				...
				<nav class="secnav">
					...
					<xsl:choose>
		    			<xsl:when test="(document/config/parameter[@name='pagetype'] = 'forms_php')" >
		    				<xsl:call-template name="formsindexfilter" />
		    			</xsl:when>
		    			<xsl:when test="(document/config/parameter[@name='pagetype'] = 'siteindex_php')" >
		    			...
		    			<xsl:otherwise>	
			    			<ul id="currnav">
								...
								<xsl:processing-instruction name="php"> include($_SERVER["DOCUMENT_ROOT"] . "<xsl:value-of select="concat($ou:sitepath,$dirname)"/>sidenav.inc"); ?</xsl:processing-instruction>
								<xsl:comment> /com.omniupdate.div </xsl:comment>
							</ul>
							<ul id="prevnav">
								<xsl:if test="($dirname != '/')" >
									<xsl:comment> com.omniupdate.div label="parentnav" path="<xsl:value-of select="$parent-path"/>/sidenav.inc" </xsl:comment> 
									<xsl:processing-instruction name="php"> include($_SERVER["DOCUMENT_ROOT"] . "<xsl:value-of select="concat($ou:sitepath,$parent-path)"/>/sidenav.inc"); ?</xsl:processing-instruction>
									<xsl:comment> /com.omniupdate.div </xsl:comment>
								</xsl:if>
							</ul>
						</xsl:otherwise>
				...
				<div class="left"> 
					<xsl:choose>
						...
						<xsl:when test="(document/config/parameter[@name='pagetype'] = 'staff_php')">
